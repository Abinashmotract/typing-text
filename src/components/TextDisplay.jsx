...
const StyledPaper = styled(Paper)(({theme}) => ({
    ...
    background: theme.palette.mode === "dark" ? "rgba(30, 30, 30, 0.7)" : "rgba(255, 255, 255, 0.7)",
    backdropFilter: "blur(10px)",
    borderRadius: 16,
    position: "relative",
    overflow: "hidden",
    border: theme.palette.mode === "dark" ? "1px solid rgba(124, 77, 255, 0.2)" : "1px solid rgba(124, 77, 255, 0.1)",
    boxShadow: theme.palette.mode === "dark" ? "0 8px 32px 0 rgba(0, 0, 0, 0.36)" : "0 8px 32px 0 rgba(0, 0, 0, 0.1)",
    ...
    const renderText = () => {
        return text.split("").map((char, index) => {
            ...
            if (index === currentIndex) {
                return (
                    <Cursor key={index}>
                        <Character correct={status === "correct"} current={true} justTyped={justTyped}>
                            {char}
                        </Character>
                    </Cursor>
                );
            ...
            return (
                <Character key={index} correct={status === "correct"} current={false} justTyped={justTyped}>
                    {char}
                </Character>
            );
        });
